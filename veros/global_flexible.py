...
class GlobalFlexibleResolutionSetup(VerosSetup):
    ...
    def set_parameter(self, vs):
        vs.identifier = 'UNNAMED' ### name for files
        ...
        vs.variables.update(
            ...
            tempNoisefield=Variable('tempNoisefield', ('xt', 'yt'), '', '', dtype='float',output=True, time_dependent=False),
            globalNoise=Variable('globalNoise', '', '', '', dtype='float',output=True, time_dependent=False),
        )
    ...
    def set_initial_conditions(self, vs):
        ...
        ### Read freshwater mask on fine grid. Then interpolate.
        ...
        vs.tempNoisefield[2:-2, 2:-2] = 0. * np.random.normal(loc=0., scale=1., size=salt_mask_data.shape)
        vs.globalNoise =0. * np.random.normal(loc=0., scale=1.)  
    ...
    def set_forcing(self, vs):
        t_rest = 720. * 86400. ### salinity restoring time
        ...
        ### noisy temperature forcing
        if (vs.time/86400.%30.==0.):
                vs.tempNoisefield = 0.*np.random.normal(loc=0., scale=1.,size=fxa.shape) + 0.*np.random.standard_normal()
                #print('NEW NOISE FIELD')
        vs.qqnec = f1 * vs.qnec[..., n1] + f2 * vs.qnec[..., n2]
        vs.qqnet = f1 * vs.qnet[..., n1] + f2 * vs.qnet[..., n2]
        vs.forc_temp_surface[...] = (vs.qqnet + vs.qqnec * (fxa + vs.tempNoisefield - vs.temp[..., -1, vs.tau])) \
        ...
        ampl = 0.00000001
        t_run = 300.
        t_ramp = 200. #ramping time in years
        t_start = 31900. #simulation time in years of spin-up
        t_sim = vs.time/360./86400.
        t_diff = (t_sim-t_start)%(t_run)
        count = math.floor((t_sim-t_start)/t_run)
        #if count>71:
        #    count = 144 - count
